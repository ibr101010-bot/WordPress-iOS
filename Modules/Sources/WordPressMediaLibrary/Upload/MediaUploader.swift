...
public actor MediaUploader {
    ...
    deinit {
        ...
        for entry in entries.values {
            if case .pending(let pending) = entry {
                pending.task.cancel()
            }
        }
    ...
    func cancelAllPending() {
        ...
        for (id, entry) in entries {
            guard case .pending(let pending) = entry else { continue }
    ...
    func dismissAllFailed() {
        var remaining = entries
        for (id, entry) in entries {
            guard case .failed(let failedEntry) = entry else { continue }
            remaining.removeValue(forKey: id)
    ...
    /// The work an entry's task performs: the full materialize-then-upload
    /// pipeline for a fresh enqueue, or upload-only for a retry of an
    /// already staged file.
    private enum WorkPhase {
        case materialize(UploadSource, overall: Progress)
        case uploadOnly(MaterializedUpload, overall: Progress)
    }

    private func beginWork(for id: UUID) -> WorkPhase? {
        guard case .pending(let entry)? = entries[id] else { return nil }
        if let source = entry.source {
            return .materialize(source, overall: entry.overallProgress)
        }
        if let materialized = entry.materialized {
            return .uploadOnly(materialized, overall: entry.overallProgress)
        }
        return nil
    }
    ...
    private func makeWorkTask(for id: UUID) -> Task<Void, Never> {
        Task { [weak self, materializer, transport] in
            do {
                guard let phase = await self?.beginWork(for: id) else { return }
                let params: MediaCreateParams
                let overall: Progress
                let uploadWeight: Double
                switch phase {
                case .materialize(let source, let overallProgress):
                ...
                case .uploadOnly(let materialized, let overallProgress):
    ...
    private func markFailed(id: UUID, error: Error) {
        ...
        if materialized != nil,
            let materializerError = error as? MaterializerError,
            case .fileNotFound = materializerError
        {
            ...
            // dismiss-only and remove any staging directory leftovers.
            Self.removeStagingDirectory(materialized?.stagingDirectory)
            materialized = nil
        }
...
private struct InternalPending {
    ...
    /// The original source, kept only until materialization completes so
    /// large in-memory payloads (e.g. camera images) are released as soon
    /// as their bytes are staged on disk.
